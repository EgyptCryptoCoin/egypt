...
#if defined(NDEBUG)
# error "Egypt cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Egypt Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s EGY additional fees, %d delta bytes\n",
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
	...
	CAmount nSubsidy;
     if  (nHeight == 1)
		 nSubsidy = 189000000 * COIN;
     else
          nSubsidy = 5 * COIN;
    // Subsidy is cut in half every 210,000 blocks which will occur approximately every 5 years.
    nSubsidy >>= halvings;
    return nSubsidy;
}
...
bool CVerifyDB::VerifyDB(const CChainParams& chainparams, CCoinsView *coinsview, int nCheckLevel, int nCheckDepth)
{
    ...
    if (nCheckDepth <= 0)
        // Egypt: suffices until year 10214. Didn't x4 value due to integer wrap around and upstream compatibility.
